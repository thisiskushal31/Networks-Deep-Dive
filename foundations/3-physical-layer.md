...
**LLDP and PoE:** Many switches use **LLDP** (or CDP on Cisco) to exchange **power** information: the PD can advertise its required power (e.g. via LLDP-MED or vendor TLVs), and the PSE can allocate or deny power. See [Data link layer – CDP and LLDP](./4-data-link-layer.md#layer-2-discovery-cdp-and-lldp) for discovery protocols.
